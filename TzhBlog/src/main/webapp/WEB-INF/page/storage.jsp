    ...
    <div style="margin-left: 28.7%;height: 70%;margin-right: 10%;background-color: white"><%--/**/#f5f5f5--%>
        <div style=" border-bottom: 0.3px solid  #000 ;width: 100%;height: 4%">
            <div style="float: left;width: 30%;height: 100%">
                <h4 style="padding-left: 10%;padding-top: 1%;">我的收藏</h4>
                <br/>
            </div>
                        ...
                        <table class="table table-hover table-responsive table-striped table-bordered"
                            ...
                            <c:forEach items="${currentPageUsers}" var="li">
                                <tr id="${li.user.username}">
                                    <td>
                                        <a href="<%=basePath%>/Blog/view_blog?id=${li.article.id}&username=${li.user.username}">${li.article.article_name}:${li.user.nickname}</a>
                                    </td>
                                    <td><fmt:formatDate type="both" value="${li.storage_time}" /></td>
                                    <td>
                                        <c:if test="${li.type=='1'}">
                                        <button class="layui-btn layui-btn-lg layui-btn-primary layui-btn-radius" onclick="can(this)"
                                        value="${li.id}" id="btn_can">取消
                                        </button>
                                        </c:if>
                                        <c:if test="${li.type=='0'}">
                                        <button class="layui-btn layui-btn-lg layui-btn-primary layui-btn-radius" onclick="storage(this)"
                                                value="${li.article_id}" id="btn_can">收藏
                                        </c:if>
                                    </td>
                                </tr>
                            </c:forEach>
                        ...
                        <div class="text-center" style="margin-top: 0px">
                            <nav>
                                <ul class="pagination">
                                    ...
                                    <li><a href="<c:url value="${basePath}/Blog/storage?page=1"/>">首页</a>
                                    </li>

                                    <li>
                                        <a href="<c:url value="${basePath}/Blog/storage?page=${page-1>1?page-1:1}"/>">&laquo;</a>
                                    </li>
                                    ...
                                    <c:forEach begin="1" end="${totalPages}" varStatus="loop">
                                        ...
                                        <li class="${active}"><a
                                                href="<c:url value="${basePath}/Blog/storage?page=${loop.index}"/>">${loop.index}</a>
                                        </li>

                                    </c:forEach>
                                    <li>
                                        <a href="<c:url value="${basePath}/Blog/storage?page=${page+1<totalPages?page+1:totalPages}"/>">&raquo;</a>
                                    </li>
                                    <li>
                                        <a href="<c:url value="${basePath}/Blog/storage?page=${totalPages}"/>">尾页</a>
                                    </li>
                                </ul>
...
<script>
    function can(obj) {
        var id=obj.value;
        window.location.href="<%=basePath%>/Blog/deleteById?id="+id;
    }
    function storage(obj) {
        var id=obj.value;
        window.location.href="<%=basePath%>/Blog/storageById?id="+id;
    }
